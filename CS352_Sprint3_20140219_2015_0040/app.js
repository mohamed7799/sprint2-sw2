...
let db = mongoose.connect(
  "mongodb+srv://mohamed:mohamed@cluster0.oetao.mongodb.net/mohamed?retryWrites=true&w=majority",
...
let notesTemps = {
  ...
  forgPass: {
    data: "this note for forgetting your password",
    type: "forgetpassowrd",
  },
...
let notification_control = {
  ...
  addNote: function (userName, userAddress, messageType, noteType) {
    ...
    this.notList.push(
      new Notification(userName, userAddress, messageTemp, noteType)
    );
  },
...
let sending_sms_notes = () => {
  let smsQueue;
  smsNote.find().then((data) => {
    console.log("----")
    smsQueue = data;
    smsQueue.forEach(note=>{
        
        console.log(`the message has been sent for the user ${note.userName} of type ${note.noteType} with content ${note.message}`)
        console.log()
    })
    console.log("the sms notes has been sent")
    console.log("----")
  });
};


let sending_mail_notes = () => {
    let mailQueue;
    mailNote.find().then((data) => {
        console.log("----")
      mailQueue = data;
      mailQueue.forEach(note=>{
        console.log(`the message has been sent for the user ${note.userName} of type ${note.noteType} with content ${note.message}`)
        console.log("")})
      console.log("the mail notes has been sent")
      console.log("----")
    });
  };
